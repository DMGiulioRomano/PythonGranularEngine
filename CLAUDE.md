...
- **The window catalogue is target-agnostic (#202):** a `WindowSpec` describes the window in its own terms — shape, coefficients, params, symmetry — and never names a GEN routine or a NumPy function. Two `WindowEmitter`s translate it: `CsoundWindowEmitter` (spec → `GenTable`, which `CsoundEmitter` writes as an `f` statement) and `NumpyWindowEmitter` (spec → `np.ndarray`); `NumpyWindowRegistry` is only the cache plus the below-threshold policy. A shape is parametric, so a fifth cosine-sum window is one catalogue line and no emitter line. Coverage is **declared** (`supports()`, `covered_names()`), never discovered at render time: `available_windows()` is what that target can produce, and a test requires every catalogue spec to be materializable by every registered emitter. "Shape" means shape *plus the fields it reads*: what a shape needs is declared beside the shapes (`WindowShape.REQUIRED_PARAMS`, `REQUIRES_COEFFICIENTS`) and derived by `missing_shape_fields()` — an incomplete description never becomes a `WindowSpec`, and every target declares it uncovered, because answering on the shape name alone meant a `TypeError` inside the NumPy formula, a `None` written into a Csound p-field, or an all-zero array with no error at all. GEN20 is a closed menu, so an arbitrary cosine sum is legitimately uncovered on Csound
- **Csound codegen lives in one place:** `rendering/csound_emitter.py` (`CsoundEmitter`) is the only module that writes `.sco` syntax — i-statements, sample/window f-statements, the FUNCTION TABLES section, the `e` end statement, and the `;` of every comment and section rule. `Grain`, `FtableManager` and `WindowRegistry` deliberately don't know any target (#203): `Grain` is the datum, `FtableManager` allocates table numbers and is the symbol table shared with the NumPy and SuperCollider back-ends, `WindowRegistry` is the shape catalogue (#202) and `CsoundWindowEmitter` its Csound translator — which returns numbers, not syntax. `ScoreWriter` takes the emitter in its constructor and only decides section order. A test reads those modules — `ScoreWriter` and the three window-emitter modules included — as AST and fails if a literal of theirs starts a score line again

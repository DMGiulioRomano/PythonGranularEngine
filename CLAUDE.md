...
- **Csound codegen lives in one place:** `rendering/csound_emitter.py` (`CsoundEmitter`) is the only module that writes `.sco` syntax — i-statements, sample/window f-statements, the FUNCTION TABLES section. `Grain`, `FtableManager` and `WindowRegistry` deliberately don't know any target (#203): `Grain` is the datum, `FtableManager` allocates table numbers and is the symbol table shared with the NumPy and SuperCollider back-ends, `WindowRegistry` is the name catalogue. `ScoreWriter` takes the emitter in its constructor and only decides section order. A test reads those three modules as AST and fails if a literal of theirs starts a score line again
